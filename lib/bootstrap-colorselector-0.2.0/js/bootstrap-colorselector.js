...
(function($) {
  ...
  ColorSelector.prototype = {
    ...
    _init : function() {
      ...
      //$("option", this.$select): 在this.$select中的option元素，不是根文档的option元素
      $("option", this.$select).each(function() {
        ...
        //具有true和false值的属性，如checked, selected或者disabled使用prop，其他的属性操作用attr
      ...
      //click.colorselector: click事件的命名空间为colorselector，$.proxy设置代理函数并传递当前this。感觉这行代码用普通的写法也行？
      $markupUl.on('click.colorselector', $.proxy(this._clickColor, this));
    },

    _clickColor : function(e) {
      
      //e.target：触发此事件的DOM对象
      var a = $(e.target);
